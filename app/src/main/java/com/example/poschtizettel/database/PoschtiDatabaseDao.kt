...
interface PoschtiDatabaseDao{
    ...
    @Transaction
    fun setState(lists: List<ShoppingList>, items: List<ShoppingItems>) {
        nukeItems()
        nukeLists()
        insertLists(*lists.toTypedArray())
    ...
    @Query("SELECT * from item_commands")
    fun getAllItemCommands(): List<ItemCommand>

    @Query("SELECT * from list_commands")
    fun getAllListCommands(): List<ListCommand>
